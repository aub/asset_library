...
You will also need a configuration file to describe the list of assets to
be included. First, tell asset_library where to find the config file:

  AssetLibrary.config_path = File.join(RAILS_ROOT, 'config', 'asset_library.yml')

In rails, you can put this line in config/initializers/asset_library.rb.
